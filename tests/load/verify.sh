...
purchased="$("${PSQL[@]}" "SELECT count(*) FROM tickets WHERE status='purchased';")"
[ "$purchased" = "$INVENTORY" ] \
    || fail "expected $INVENTORY purchased tickets, found $purchased"
pass "exactly $purchased tickets purchased"

available="$("${PSQL[@]}" "SELECT count(*) FROM tickets WHERE status='available';")"
...
orphaned="$("${PSQL[@]}" "SELECT count(*) FROM tickets WHERE status='purchased' AND user_id IS NULL;")"
